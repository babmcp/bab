...
After Claude completes the investigation, it automatically calls the selected AI model (unless confidence is **certain**, in which case expert analysis is bypassed):
...
- **Confidence assessment and revision** capabilities across six granularity levels
...
- `step_number`: Current step number in investigation sequence, >= 1 (required)
- `total_steps`: Estimated total investigation steps, >= 1 (required, adjustable as process evolves)
- `next_step_required`: Whether another investigation step is needed (required)
- `findings`: Clues and evidence collected in this step (required)
...
- `relevant_context`: Specific methods/functions involved in the issue
- `issues_found`: Issues identified with description and severity (array of `{description, severity}`)
- `hypothesis`: Current best guess about the underlying root cause
- `confidence`: Confidence level in current hypothesis (exploring/low/medium/high/very_high/almost_certain/certain)
