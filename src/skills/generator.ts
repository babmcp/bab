  ...
  const sections = pluginRecords
    .map((p) => {
      ...
      const capsList = [
        ...
        `Output format: ${caps?.output_format ?? "unknown"}`,
      ];

      return `## \`${p.manifest.id}\` — ${p.manifest.name}

**CLI command:** \`${p.manifest.command}\`
**Source:** ${p.sourceType}${p.installMetadata ? ` (${p.installMetadata.source_original})` : ""}
