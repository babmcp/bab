...
import type { DelegateEvent, ToolError, ToolOutput } from "../../types";
...
export function createDelegateTool(config: BabConfig): RegisteredTool {
  ...
  async function getLoadedPlugins(): Promise<
    Awaited<ReturnType<typeof loadPlugins>>
  > {
    if (pluginCache && Date.now() - pluginCache.at < PLUGIN_CACHE_TTL_MS) {
    ...
    pluginCache = { loaded, at: Date.now() };
    return loaded;
  }
